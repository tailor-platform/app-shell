...
- The `app-shell-patterns` consumer skill is emitted by `docs-kit sync` (see `packages/docs-kit/src/skill.ts`) from `docs-src/` — components, hooks/api, patterns, pages, plus the authored fundamentals in `docs-src/skill/` — so it refreshes automatically on every resync; there is no separate step.
