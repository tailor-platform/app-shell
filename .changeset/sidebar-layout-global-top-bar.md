...
- `SidebarLayout` gains a `topBar` slot, rendered above the sidebar + content row. The fixed sidebar is offset to start just beneath it (via `--appshell-topbar-h`, `0px` when there is no `topBar`). The bar should be `3.5rem` tall.
...
<SidebarLayout
  topBar={<GlobalTopBar />}
  sidebar={<SidebarLayout.DefaultSidebar hideHeader iconRail />}
/>
```
