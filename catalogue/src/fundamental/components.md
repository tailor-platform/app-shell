...
| JSX examples tied to ERP patterns (`patterns/*`)                                                                  | **Styling** rules — plain utilities everywhere, `!` to override an AppShell default               |
...
**Notes:** Children that aren't `Layout.Header` or `Layout.Column` are filtered out. Column gap overrides (`<Layout className="gap-6!" />` — the `!` is required, the root sets its own `gap-4`) → **`design-system.md`** §5 (styling rules).
...
**Notes:** Size the panel with `Sheet.Content`'s **`size`** prop (`"sm" | "md" | "lg" | "xl" | "full"`, default `"sm"`) — not a width utility. Styling rules → **`design-system.md`** §5.
...
- **Inside a card?** Pass `containerClassName="px-6"` on `Table.Root` for the horizontal inset, and either drop `Card.Content` (bare list form) or pass `Card.Content className="px-0!"` (header+content form — the `!` is required, `Card.Content` sets its own `px-6`). Skipping the `containerClassName` lands the first column flush against the card edge. See the `Card` entry for the two canonical forms and a DON'T example. Dense cell typography (**`text-sm`**, **`tabular-nums`** for figures, **`font-mono`** for identifiers) → **`design-system.md`** §4 Typography.
...
- **Tables inside a card need TWO co-requisite geometry changes** (token-backed spacing rationale → **`design-system.md`** §4 Spacing): (a) Card stops imposing horizontal padding — drop `Card.Content` for the bare form, or pass `Card.Content className="px-0!"` for the header+content form (the `!` overrides `Card.Content`'s own `px-6`). (b) `Table.Root` provides the inset itself via `containerClassName="px-6"` — an addition, so no `!` needed. Skipping (b) lands the first column flush against the card edge — `Table.Cell`'s intrinsic first-child padding does NOT render reliably in this composition.
...
**Sizing:** no fixed height — `rows` sets the visible line count and the user can drag it taller. `min-h-16` is a hard floor, so `rows` only takes effect from 3 up. Width is the parent's job: the control is `w-full`, and `cols` is not accepted because it could never win against that. Do **not** reach for `Input` and a height override; `Input` is locked to `h-9`. Use `Textarea` and `rows`.
