/**
 * Re-vendors the Noto Sans JP variable subsets.
 *
 * NOT part of the build. Its output — `src/assets/fonts/noto-sans-jp.css` and
 * `src/assets/fonts/files/noto-sans-jp-*.woff2` — is COMMITTED, matching how
 * `inter.css` and the Inter subsets are vendored. Run it by hand when the pinned
 * `@fontsource-variable/noto-sans-jp` devDependency is deliberately bumped:
 *
 *   pnpm --filter @tailor-platform/app-shell vendor:fonts
 *   git add src/assets/fonts
 *
 * It exists rather than the output being hand-maintained because the two
 * non-obvious transforms below — clamping every subset to JP_BLOCKS, and the
 * fallback weight ladder — are where four review defects lived. Re-deriving them
 * by hand on the next bump would be a good way to reintroduce them.
 *
 * Idempotent: re-running with the same upstream version produces byte-identical
 * output and copies nothing.
 */
 ...
 *
 * ── Why the descriptors look like this ──────────────────────────────────────
 *
 * size-adjust: ${SIZE_ADJUST}% — Noto's kanji run ~1.09x Inter's cap height, which reads
 *   oversized beside Inter's large x-height in mixed strings. Picked from a rendered
 *   sweep (88/90/92/94/96/100): above ~96% the Japanese dominates, below ~90% it
 *   looks undersized. It is an aesthetic call, not a derived constant, and it is
 *   also the whole ~6% advance-width delta versus the OS font — Noto at 100%
 *   measures within 0.2% of Hiragino, since CJK glyphs sit on a full-width em grid.
 *
 * ascent/descent-override: pre-divided by size-adjust, because size-adjust scales
 *   the override percentages too (97 x 0.94 = 91, not 97). ${pct(INTER_ASCENT)}/${pct(INTER_DESCENT)} lands the
 *   face on Inter's actual 97%/24% box, so a Japanese line is exactly as tall as a
 *   Latin one.
 *
 * The fallback ladder is bucketed per weight. A single \`100 500\` bucket pointing at
 *   Regular faces made \`font-weight: 500\` render as Regular — the exact bug this
 *   font fixes — and regressed macOS, because this family sits ahead of
 *   \`ui-sans-serif\` and so pre-empts the cascade that would have found Hiragino W5.
 *
 * Both families are clamped to Japanese blocks. Upstream's ranges also cover
 *   arrows, dingbats and letterlike symbols shared with Latin, so leaving them in
 *   meant a Latin-only app fetched a Japanese subset the first time it rendered the
 *   DataTable's boolean "✓" (U+2713).
 *
 * ── Regenerating ────────────────────────────────────────────────────────────
 * GENERATED by scripts/sync-noto-fonts.mjs — do not edit by hand. The output is
 * committed (as Inter's is); the script is a manual re-vendoring tool, not a build
 * step. Bump the pinned devDependency, run
 * \`pnpm --filter @tailor-platform/app-shell vendor:fonts\`, commit the result.
 *
