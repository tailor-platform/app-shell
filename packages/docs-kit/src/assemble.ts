...
function renderApi(opts: {
  ...
  const { surface, owned, claims, kind, upstream } = opts;
  const sections: string[] = [];

  // Claims are orthogonal to kind, so a code-backed unit can carry both a
  // generated prop table and a re-export list.
  if (claims.length > 0) {
    const lines = claims.map((n) => `- \`${n}\``).join("\n");
  ...
  if (kind !== "code-backed" || owned.length === 0) return sections.join("\n\n");
  ...
  for (const name of owned) {
    ...
    const body = sym.props
      .map((p) => {
        ...
        return `| \`${p.name}${p.optional ? "?" : ""}\` | \`${cell(p.type)}\` | ${def} | ${cell(p.description)} |`;
      })
      .join("\n");
    tables.push(`### ${name}\n\n${head}\n${body}`);
  }
...
export function assembleMarkdown(opts: {
  ...
  const { title, description } = outline.frontmatter;
  ...
  const api = renderApi({
    ...
    kind: outline.kind,
    upstream: outline.frontmatter.upstream,
  });

  // The generated API block is OPT-IN: it renders only where the outline places
  // an `<!-- api -->` token. Simple components (one `*Props`) opt in; compound
